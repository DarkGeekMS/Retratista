...
class BertForMultiLable(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # classifier to use for binary multi-label classification
        self.classifier_pos_neg = nn.Linear(config.hidden_size, config.num_labels)
        # classifier to use to check existence of labels in the text for binary multi-label classification
        self.classifier_extistence = nn.Linear(config.hidden_size, config.num_labels)
        # regressor to use for get a scale in each label
